...
逐节点说明 On Error、Retry On Fail 与 error output 连线。导入后若 UI 未显示错误分支，请按本手册核对。
...
JSON 中的 `errorWorkflow` 为名称字符串，**导入后 n8n 通常不会自动绑定**。请对每个主工作流手动设置：

**Settings → Error Workflow → `B2B Lead Error Handler`**

适用于：`B2B Lead Intake`、`B2B Lead Enrichment Scoring`、`B2B Lead CRM Sync Notification`、`B2B Lead Daily Summary`、`B2B Lead Weekly Summary`、`B2B Lead Booking Follow-up`、`B2B Lead Calendly Webhook`、`B2B Lead Slack Actions`

### 凭证占位符

仓库内 Google Sheets 节点使用 `GOOGLE_SHEETS_CREDENTIAL_ID` 占位符。**n8n export 会将 credential 解析为真实 ID**（如 `ITJp5Xyp9z6SOvYV`），属正常行为；重新导入后需重新绑定凭证。
...
所有 `Handle * Error` Code 节点从 `$input.item.error` 解析 `errorMessage`，并写入对应的 `*_error_message` 字段与 `_metadata.error_message`。Handler 通过 `$('上游Node').first()?.json` 保留 lead 上下文，避免 Sheets/HTTP 响应覆盖业务对象。
...
| **Read config_main** | Continue (error output) | ON, 3x, 5s | → Handle config_main Read Error（降级为默认阈值） |
| **Read config_notifications** | Continue (error output) | ON, 3x, 5s | → Handle config_notifications Read Error（默认关闭 daily Slack） |
| **Handle Summary Read Error** | Stop | OFF | → `summary_read_error_message`；`daily_gate_passed=false` |
| Build Daily Summary | Stop | OFF | `runOnceForAllItems` + `safeNodeAll`；阈值读 `score_threshold_high` |
| **Slack Daily Report** | Continue (error output) | ON, 3x, 5s | → Log Slack Summary Error → `slack_error_message`（保留 summary） |

**门控：** 仅当 `config_main.mode=production` 且 `config_notifications.daily_summary.enabled=true` 时发送 Slack；test/disabled 仍聚合 summary，走 `Skip Daily Slack`。

**窗口：** 每日 09:00 触发；汇总**昨日完整 UTC 日**（`date` = 昨日 UTC `YYYY-MM-DD`），不是触发日当天的 00:00–09:00。

**拓扑：**

```text
Daily 9am → Read Leads → Read Errors
  → Read config_main → Read config_notifications → Load Daily Config
      → Build Daily Summary → Should Send Daily? → Slack / Skip → End
```

任意 read 失败时，`Handle Summary Read Error` 使用 `safeNodeAll` 保留已成功读取的 leads；门控关闭，因而跳过 Slack。
...
| **Read config_main** | Continue (error output) | ON, 3x, 5s | → Handle config_main Read Error（降级为默认阈值） |
| **Read config_notifications** | Continue (error output) | ON, 3x, 5s | → Handle config_notifications Read Error（默认关闭 weekly Slack） |
| **Handle Weekly Read Error** | Stop | OFF | → `weekly_read_error_message`；降级 metrics → HTTP Weekly Insights |
| Build Weekly Metrics | Stop | OFF | 门控用严格 `weekly_summary_enabled === true` |
| **HTTP Weekly Insights** | Continue (error output) | ON, 3x, 5s | → Handle Weekly AI Failure → 仅 metrics 的 `ai_summary` fallback |
| **Slack Weekly Report** | Continue (error output) | ON, 3x, 5s | → Log Slack Weekly Error（保留 Merge metrics）或 Restore → Append |
| **Append Weekly Metrics** | Continue (error output) | ON, 3x, 5s | → Handle Append Weekly Metrics Error → End；供下周 WoW 的历史行 |
...
Friday 5pm → Read Leads → Read Errors → Read Prior weekly_metrics
  → Read config_main → Read config_notifications → Load Weekly Config
      → Build Weekly Metrics → HTTP Weekly Insights → Merge Weekly Report
          → Should Send Weekly? → Slack (Restore) / Skip / Slack-error (preserve metrics)
              → Append weekly_metrics → End
```

Read 失败时：`Handle Weekly Read Error` 跳过 Build Weekly Metrics，直接调用 HTTP Weekly Insights（AI 失败则 metrics-only fallback）。
Slack 失败时：`Log Slack Weekly Error` 从 `Merge Weekly Report` 还原 metrics 再 Append，避免冲掉周报行。
Append 失败时：走 error 分支到 `Handle Append Weekly Metrics Error` 后 End（不会挂起运行）。
...
| **Read config_main** | Continue (error output) | ON, 3x, 5s | → Handle config_main Read Error（降级为默认阈值） |
| **Read config_notifications** | Continue (error output) | ON, 3x, 5s | → Handle config_notifications Read Error（默认关闭 reminder） |
...
5. 断开 Sheets 凭证跑一次 Intake → 应降级而非整工作流崩溃
...
| Daily Summary read 失败 | `summary_read_error_message` → 降级 summary，`daily_gate_passed=false`，Skip Slack |
| Daily / Weekly Summary test 模式 | 聚合指标；**不**发 Slack（需 `mode=production` + 对应 `enabled=true`） |
| Weekly Summary read 失败 | `weekly_read_error_message` → 降级 metrics，仍写 weekly_metrics |
| Weekly Slack 失败 | `Log Slack Weekly Error` 保留 Merge metrics → 仍 Append `weekly_metrics` |
| Weekly Append `weekly_metrics` 失败 | `Handle Append Weekly Metrics Error` → End（不会挂起运行） |
